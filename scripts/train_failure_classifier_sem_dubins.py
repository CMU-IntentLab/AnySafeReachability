...
def wandb_setup():
    ...
    wandb.init(name=config.wandb_name, project="Semantic Encoder")
...
param_groups = [
    {
        ...
        "lr": float(config.sem["lr"]) * 1,
    },
]
# Optimizer Setting
opt = torch.optim.AdamW(
    param_groups, lr=float(config.sem["lr"]), weight_decay=config.sem["weight_decay"]
)

scheduler = torch.optim.lr_scheduler.StepLR(
    opt, step_size=config.sem["lr_decay_step"], gamma=config.sem["lr_decay_gamma"]
)

# Dataset Loader and Sampler
BS = config.sem["sz_batch"]  # batch size
BL = config.batch_length  # batch length

# print("Training parameters: {}".format(vars(args)))
print("Training for {} epochs.".format(config.sem["nb_epochs"]))
losses_list = []
best_epoch = 0
best_eval = -float("inf")
...
for epoch in tqdm(range(0, config.sem["nb_epochs"]), desc="Training Epochs", position=0):
    model.train()
    ...
    if epoch >= 0:
        ...
        if config.sem["save_model"]:
            model_name = config.wandb_name

            save_path = "logs/checkpoints_sem"
            ...
            if True:  # balanced_accuracy < best_eval:
                ...
                torch.save(
                    model.state_dict(),
                    f"logs/checkpoints_sem/best_encoder_{model_name}.pth",
                )
